/**
 * @file calc.y
 * @beief This file is part of the clc distribution-
 * Copyright (c) 2026 blu3
 *
 * A fixed and completed parser with an interpreter for a simple functional
 * language. This file (calc.y) defines the grammar for the a small functional
...
expr:
    ...
    | lambda_expr
    | block_expr
    /* The rules below build binary operation AST nodes. */
    | expr T_OR expr        { $$ = make_node(NODE_BINOP); $$->binop.left=$1; $$->binop.right=$3; $$->binop.op=T_OR; }
    | expr T_AND expr       { $$ = make_node(NODE_BINOP); $$->binop.left=$1; $$->binop.right=$3; $$->binop.op=T_AND; }
    | expr T_EQ expr        { $$ = make_node(NODE_BINOP); $$->binop.left=$1; $$->binop.right=$3; $$->binop.op=T_EQ; }
    | expr T_NEQ expr       { $$ = make_node(NODE_BINOP); $$->binop.left=$1; $$->binop.right=$3; $$->binop.op=T_NEQ; }
    | expr T_LT expr        { $$ = make_node(NODE_BINOP); $$->binop.left=$1; $$->binop.right=$3; $$->binop.op=T_LT; }
    | expr T_GT expr        { $$ = make_node(NODE_BINOP); $$->binop.left=$1; $$->binop.right=$3; $$->binop.op=T_GT; }
    | expr T_LTE expr       { $$ = make_node(NODE_BINOP); $$->binop.left=$1; $$->binop.right=$3; $$->binop.op=T_LTE; }
    | expr T_GTE expr       { $$ = make_node(NODE_BINOP); $$->binop.left=$1; $$->binop.right=$3; $$->binop.op=T_GTE; }
 ...
 */
let_expr:
    T_LET T_SYM T_SET expr T_IN expr {
        $$ = make_node(NODE_LET);
        $$->let.name = $2;
        $$->let.value = $4;
        $$->let.body = $6;
...
Value *eval(ASTNode *node, Env *env) {
    ...
    switch(node->type) {
        ...
        case NODE_LET: {
            ...
            free_env(local, false); // Free the local scope.
            return retval;
        }
